...
from AniPortrait.src.models.pose_guider import PoseGuider as poseguider
from dataset_related.dataset_write_4 import MyDataset,collate_fn # 这里别忘记改了
"""
train_dataloader在不同的训练阶段是有不同表现形式的，在第一阶段没有引入motion module的时候我们采用的其实是图片-图片的训练方式
"""

"""
配合dataset_write_4 使用"""
...
"""
使用卷积网络对三个特征进行融合，在这里我们加上残差连接，强调手部条件信息
在这个版本中，我们加上了训练时单步推理的过程
"""
class FusionNet(nn.Module):
    def __init__(self,in_channels=320*3,out_channels=320,kernel_size=3):
        super().__init__()
        self.layer_1=nn.Sequential(nn.Conv3d(in_channels, out_channels, kernel_size=kernel_size, padding=kernel_size//2),nn.BatchNorm3d(out_channels),nn.ReLU())
        self.layer_2=nn.Sequential(nn.Conv3d(in_channels, out_channels, kernel_size=kernel_size, padding=kernel_size//2),nn.BatchNorm3d(out_channels),nn.ReLU())
        self.layer_3=nn.Sequential(nn.Conv3d(in_channels*2, out_channels*2, kernel_size=kernel_size, padding=kernel_size//2),nn.BatchNorm3d(out_channels*2),nn.ReLU())
        self.layer_4=nn.Sequential(nn.Conv3d(in_channels*4, out_channels*4, kernel_size=kernel_size, padding=kernel_size//2),nn.BatchNorm3d(out_channels*4),nn.ReLU())
        self.layer_5 = nn.Sequential(nn.Conv3d(in_channels * 4, out_channels * 4, kernel_size=kernel_size, padding=kernel_size // 2),nn.BatchNorm3d(out_channels * 4), nn.ReLU())

    def forward(self,mesh_list,body_list,hand_list):
        a=self.layer_1(torch.cat([mesh_list[0],body_list[0],hand_list[0]],dim=1))+hand_list[0]
        b=self.layer_2(torch.cat([mesh_list[1],body_list[1],hand_list[1]],dim=1))+hand_list[1]
        c=self.layer_3(torch.cat([mesh_list[2],body_list[2],hand_list[2]],dim=1))+hand_list[2]
        d=self.layer_4(torch.cat([mesh_list[3],body_list[3],hand_list[3]],dim=1))+hand_list[3]
        e=self.layer_5(torch.cat([mesh_list[4],body_list[4],hand_list[4]],dim=1))+hand_list[4]
        return [a,b,c,d,e]
...
class Net(nn.Module):
    def __init__(self,
                 ...
                 face_pose_guider: poseguider,  # 三个pose_guider
                 body_pose_guider: poseguider,
                 hand_pose_guider: poseguider,
        ...
        self.face_pose_guider=face_pose_guider
        self.body_pose_guider=body_pose_guider
        self.hand_pose_guider=hand_pose_guider
    ...
    def forward(self,
        ...
        mesh_fea=self.face_pose_guider(pose_mesh_tensor,ref_mesh_tensor)
        keypoint_fea=self.body_pose_guider(pose_keypoint_tensor,ref_keypoint_tensor)
        hand_fea=self.hand_pose_guider(pose_hand_tensor,ref_hand_tensor)

        # 在这里我们做融合的方式还是很简单，在这里我们做特征融合的方式仅仅是将三者相加
        pose_fea=self.fusion_net(mesh_fea,keypoint_fea,hand_fea)
...
def main(cfg):
    ...
    # 初始化poseguider，这里我们分为三个pose_guider
    face_pose_guider=poseguider().to(device=accelerator.device)
    body_pose_guider=poseguider().to(device=accelerator.device)
    hand_pose_guider=poseguider().to(device=accelerator.device)

    # 初始化fusion_net
    fusion_net=FusionNet().to(device=accelerator.device)
    ...
    face_pose_guider.requires_grad_(True)  # pose_guider的参数是需要训练的
    hand_pose_guider.requires_grad_(True)
    body_pose_guider.requires_grad_(True)
    ...
    net=Net(reference_unet,denoising_unet,face_pose_guider,body_pose_guider,hand_pose_guider,fusion_net,reference_control_writer,reference_control_reader)
    ...
    for epoch in range (first_epoch,num_train_epochs):
        ...
        for step,batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                model_pred=net(noisy_latents,timesteps,ref_img_latents,image_prompt_embeds,ref_img_mesh,ref_img_keypoint,ref_image_hands,pose_img_mesh,pose_img_keypoint,pose_image_hands)# 这里先欠着，后来检查的时候这里应该是少了一个uncond的参数

                """我们在训练的时候单步推理了一下"""
                if global_step>20000:
                    latents_pred=train_noise_scheduler.step(model_pred,train_noise_scheduler.timesteps,noisy_latents,return_dict=False)[0].to(torch.float16)
                    if (global_step % 1000==0):
                        for i in range(0,latents_pred.shape[0]):
                            image_test=decode_latents(latents_pred[i].unsqueeze(0),vae=vae)
                            image_test=torch.from_numpy(image_test)
                            image = image_test[0, :, 0].permute(1, 2, 0).cpu().numpy()
                            res_image_pil = Image.fromarray((image * 255).astype(np.uint8))
                            res_image_pil.save(f"/home/shipu/mycode/latents_see_1/image_{i}.jpg")
        ...
        if (epoch+1)% cfg.save_model_epoch_interval==0 and accelerator.is_main_process:
            ...
            save_checkpoint(model=unwrap_net.face_pose_guider,save_dir=save_dir,prefix="face_pose_guider",ckpt_num=global_step,total_limit=3)
            save_checkpoint(model=unwrap_net.body_pose_guider, save_dir=save_dir, prefix="body_pose_guider",ckpt_num=global_step, total_limit=3)
            save_checkpoint(model=unwrap_net.hand_pose_guider, save_dir=save_dir, prefix="hand_pose_guider",ckpt_num=global_step, total_limit=3)
...
if __name__ =="__main__":
    ...
    parser.add_argument("--config",type=str,default="/home/shipu/mycode/train_config/train_1_para.yaml")
...
# CUDA_VISIBLE_DEVICES=6,7 accelerate launch --config_file /home/shipu/mycode/default_config.yaml train_1_7.py
