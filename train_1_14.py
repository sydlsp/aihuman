...
"""
我们使用RealiseDance的条件融合策略，使用这个poseguider来处理三个控制条件
"""
...
class Net(nn.Module):
    def __init__(self,
                 ...
                 denoising_unet: HackUNet3DConditionModel,
                 pose_guider,
    ...
    def forward(self,
        ...
        pose_fea = self.pose_guider(pose_mesh_tensor, pose_keypoint_tensor, pose_hand_tensor)

        pose_fea = [rearrange(pe, "(b f) c h w -> b c f h w", f=frame) for pe in pose_fea]

        if not uncond_fwd:
            ref_timesteps = torch.zeros_like(time_steps)
            self.reference_unet(ref_image_latents, ref_timesteps, encoder_hidden_states=clip_image_embeds,
                                return_dict=False)
        ...
        model_pred = self.denoising_unet(noisy_latents, time_steps, latent_pose=pose_fea,
                                         encoder_hidden_states=clip_image_embeds).sample
...
def main(cfg):
    ...
    denoising_unet = HackUNet3DConditionModel.from_pretrained_2d("/data/shipu/redance_weight/rv-5-1", subfolder="unet",
                                                                 unet_additional_kwargs={"use_motion_module": False,
                                                                                         ...
                                                                                         "unet_use_cross_frame_attention": False, }).to(
        accelerator.device)
    # 图像编码器
    image_enc = CLIPVisionModelWithProjection.from_pretrained(cfg.image_encoder_path, ).to(device=accelerator.device,
                                                                                           dtype=weight_dtype)

    # 用realisedance的poseguider
    pose_guider = get_poseguider(image_finetune=True, num_conds=3).to(
        device=accelerator.device)  # 第一阶段image_finetune为true

    # 下面规定哪些参数是要训练的，哪些参数是要冻结的
    vae.requires_grad_(False)
    image_enc.requires_grad_(False)

    denoising_unet_ckpt = torch.load("/data/shipu/redance_weight/redance_unet_main.pth")
    denoising_unet.load_state_dict(denoising_unet_ckpt)
    ...
    reference_control_writer = ReferenceAttentionControl(reference_unet,
                                                         do_classifier_free_guidance=do_classifier_free_guidance,
                                                         mode="write", fusion_blocks="full", )
    reference_control_reader = ReferenceAttentionControl(denoising_unet,
                                                         do_classifier_free_guidance=do_classifier_free_guidance,
    ...
    if cfg.solver.scale_lr:
        print("here")
        learning_rate = (
                    cfg.solver.learning_rate * cfg.solver.gradient_accumulation_steps * cfg.train_bs * accelerator.num_processes)
    else:
    ...
    (net, optimizer, train_dataloader, lr_scheduler) = accelerator.prepare(net, optimizer, train_dataloader,
                                                                           lr_scheduler)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                if cfg.noise_offset > 0.0:
                    noise += cfg.noise_offset * torch.randn((noise.shape[0], noise.shape[1], 1, 1, 1),
                                                            device=noise.device)
                ...
                model_pred = net(noisy_latents, timesteps, ref_img_latents, image_prompt_embeds, pose_img_mesh,
                                 pose_img_keypoint, pose_image_hands)  # 这里先欠着，后来检查的时候这里应该是少了一个uncond的参数
                ...
                if cfg.snr_gamma == 0:
                    ...
                    mse_loss_weights = (
                                torch.stack([snr, cfg.snr_gamma * torch.ones_like(timesteps)], dim=1).min(dim=1)[
                                    0] / snr)
        ...
        if (epoch + 1) % cfg.save_model_epoch_interval == 0 and accelerator.is_main_process:
            ...
            save_checkpoint(model=unwrap_net.reference_unet, save_dir=save_dir, prefix="reference_unet",
                            ckpt_num=global_step, total_limit=3)
            save_checkpoint(model=unwrap_net.denoising_unet, save_dir=save_dir, prefix="denoising_unet",
                            ckpt_num=global_step, total_limit=3)
            save_checkpoint(model=unwrap_net.pose_guider, save_dir=save_dir, prefix="pose_guider", ckpt_num=global_step,
                            total_limit=3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="/home/shipu/mycode/train_config/train_1_test.yaml")
    # "/home/shipu/mycode/train_config/train_1_para.yaml"
...
# CUDA_VISIBLE_DEVICES=6,7 accelerate launch --config_file /home/shipu/mycode/default_config.yaml train_1_13.py
# CUDA_VISIBLE_DEVICES=7 accelerate launch --config_file  /home/shipu/mycode/one_gpu_default_config.yaml train_1_14.py
