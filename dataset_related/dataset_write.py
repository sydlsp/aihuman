...
class MyDataset(Dataset):
    ...
    def __init__(self,data_dir,sample_size=[512,512],sample_stride=4,sample_n_frames=16,is_image=False):
    ...
    def get_batch_wo_pose(self,index):
        ...
        # 这里是排序完了的列表
        frame_list=self.sort_file_list([file for file in video_file if "frame" in file])  # 帧文件名 这里有问题值得注意这个frame_list应该是看不到的
        ...
        # 在这里我们似乎还要读取ref_img的pose和mesh信息，处理方式是一样的，只是不要转化为PIL那一步
...
# mydataset=MyDataset("/remote-home/share/yfsong/shipu/dataset_video_true",is_image=False)
# pixel_values,pixel_values_mesh,pixel_values_keypoint,clip_ref_image,pixel_values_ref_img,pixel_values_ref_mesh,pixel_values_ref_keypoint=mydataset.get_batch_wo_pose(0)
#
# print(pixel_values.shape)

# 我检查了一下，上面的dataloader应该没什么问题
