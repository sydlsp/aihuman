...
"""
在dataset2的基础上，我们引入hand_bank来加强手部的控制
具体来讲：ref_hand和pose_hand都是同样的，只是pose_hand复制了k次和pose_mesh.pose_keypoint一样
"""
class MyDataset(Dataset):
    def __init__(self,data_dir,sample_size=[512,512],sample_stride=4,sample_n_frames=16,is_image=False,sample_stride_aug=4):
        ...
        # 关于手部新补充的部分
        self.hand_bank_dir=os.path.join(data_dir,'hands')  # hand文件夹，里面全是手部的图片

        self.hand_bank_length=len(os.listdir(self.hand_bank_dir))  # 拿到手部图片的总数
    ...
    def get_batch_wo_pose(self,index):
        ...
        # 补充手部相关的东西
        hand_bank_index=random.randint(0,self.hand_bank_length-1)  # 取一个序号出来
        hand_path=os.path.join(self.hand_bank_dir,f"{hand_bank_index}.jpg")  # 拿到手部图片的路径
        ...
        # 读取手部的数据
        ref_hand=cv2.imread(hand_path)
        ref_hand=cv2.cvtColor(ref_hand,cv2.COLOR_BGR2RGB)
        ref_hand=self.contrast_normalization(ref_hand)
        pixel_values_ref_hand=torch.from_numpy(ref_hand).permute(2,0,1).contiguous()
        pixel_values_ref_hand=pixel_values_ref_hand/255.
        ...
        # 视频帧的hand数据
        images_hand=[cv2.imread(hand_path) for _ in range(len(batch_idx))]
        images_hand=[cv2.cvtColor(bgr_image,cv2.COLOR_BGR2RGB) for bgr_image in images_hand]
        image_hand_np=np.array([self.contrast_normalization(img) for img in images_hand])

        pixel_values_hand=torch.from_numpy(image_hand_np).permute(0,3,1,2).contiguous()
        pixel_values_hand=pixel_values_hand/255.0

        if self.is_image:
            pixel_values=pixel_values[0]
            pixel_values_mesh=pixel_values_mesh[0]
            pixel_values_keypoint=pixel_values_keypoint[0]
            pixel_values_hand=pixel_values_hand[0]
        ...
        return pixel_values,pixel_values_mesh,pixel_values_keypoint,pixel_values_hand,clip_ref_image,pixel_values_ref_img,pixel_values_ref_mesh,pixel_values_ref_keypoint,pixel_values_ref_hand
    ...
    def __getitem__(self, item):
        """"""
        pixel_values, pixel_values_mesh, pixel_values_keypoint,pixel_values_hand, clip_ref_image, pixel_values_ref_img, pixel_values_ref_mesh, pixel_values_ref_keypoint,pixel_values_ref_hand = self.get_batch_wo_pose(item)

        # 其实过了pixel_transforms之后，图片的形状才真正改变
        ...
        pixel_values_hand=self.pixel_transforms(pixel_values_hand)
        ...
        pixel_values_ref_hand=pixel_values_ref_hand.unsqueeze(0)
        pixel_values_ref_hand=self.pixel_transforms(pixel_values_ref_hand)
        pixel_values_ref_hand=pixel_values_ref_hand.squeeze(0)
        ...
        sample = dict(
            ...
            pixel_values_ref_hand=pixel_values_ref_hand,
...
def collate_fn(data):
    ...
    pixel_values_hand=torch.stack([example["pixel_values_hand"] for example in data])
    ...
    pixel_values_ref_hand=torch.stack([example["pixel_values_ref_hand"] for example in data])
...
#     data_dir='/remote-home/share/yfsong/shipu/dataset_video_true_1'
