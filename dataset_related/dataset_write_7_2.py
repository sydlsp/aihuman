...
class MyDataset(Dataset):
    ...
    def get_batch_wo_pose(self,index):
        ...
        start_index=random.randint(0,video_length-clip_length)

        # 这里我们测过了即使 clip_length < sample_n_frames 代码也是正确的，不会抛出异常
        batch_idx=np.linspace(start_index,start_index+clip_length-1,self.sample_n_frames,dtype=int)


        # 下面我们开始处理ref_img相关
        ref_img_file_idx=random.randint(0,video_length-1)
