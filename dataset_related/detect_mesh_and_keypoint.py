...
from smirk.src.smirk_generator import SmirkGenerator
from smirk.src.FLAME.FLAME import FLAME
from smirk.src.renderer.renderer import Renderer
from smirk.datasets.base_dataset import create_mask
from smirk.utils.mediapipe_utils import run_mediapipe
import smirk.src.utils.masking as masking_utils
...
class DetectMeshandKeypoint():
    ...
    def __init__(self):
        ...
        # 初始化smirk的一系列东西
        checkpoint_path = "/data/shipu/smirk/pretrained_models/SMIRK_em1.pt"
        ...
        # 加载flame模型
        flame_model_path = "/data/shipu/smirk/assets/FLAME2020/generic_model.pkl"
        flame_lmk_embedding_path = "/data/shipu/smirk/assets/landmark_embedding.npy"
...
class DetectMeshandKeypoint_1():

    """
    在这个版本中，我们不将mesh图和keypoint图放到原来帧图片的分辨率大小了，我们直接变成256*256的大小
    同时我们把手部关键点也给加上去
    """
    ...
    def __init__(self):
        ...
        # 设置mediapipe绘图规格
        self.landmark_drawing_spec = self.mp_drawing.DrawingSpec(color=(0,255,0), thickness=4, circle_radius=5)
    ...
    def get_mesh_and_keypoint_pic(self,video_path,out_dir):
        ...
        if os.path.exists(video_path):
            ...
            while True:
                ...
                # 在这里生成一张全黑的图像，在这里我们修改一下全黑图像的尺寸，和mesh图的尺寸大小相匹配，看看这里形状会不会有偏差
                black_image=np.zeros((orig_image_height, orig_image_width,3),dtype=np.uint8)
                # 绘制关键点
                if pose_results.pose_landmarks:
                    self.mp_drawing.draw_landmarks(black_image, pose_results.pose_landmarks, mp.solutions.pose.POSE_CONNECTIONS, self.landmark_drawing_spec,self.landmark_drawing_spec)
                ...
                #         self.mp_drawing.draw_landmarks(black_image, hand_landmarks, mp.solutions.hands.HAND_CONNECTIONS,
                #                                        self.landmark_drawing_spec, self.landmark_drawing_spec)
                ...
                rendered_img_numpy=cv2.cvtColor(rendered_img_numpy,cv2.COLOR_BGR2RGB)
                keypoint_pic=cv2.cvtColor(black_image,cv2.COLOR_BGR2RGB)


                cv2.imwrite(os.path.join(out_dir,mesh_name),rendered_img_numpy)
                cv2.imwrite(os.path.join(out_dir,keypoint_name),keypoint_pic)
                cv2.imwrite(os.path.join(out_dir,frame_name),frame)
                frame_count+=1
