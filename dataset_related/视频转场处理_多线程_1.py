...
"""
  使用PySceneDetect检测视频中的场景变化，之前的处理模式还是说处理的成品率太低了，我们调整一下写法
"""
...
def split_video_and_detect_person(video_path,model,output_path,segment_duration=5.0,interval=2,person_threshold=0.3,time_threshold=0.8,count_list=[0]):
    """
       video_path: 要检测的视频本身路径
       output_path: 写到segments文件夹下
    """
    # 加载视频
    ...
    name_start=Path(output_path).parent.name
    ...
    for start_time in range(0, int(video_file_duration), int(segment_duration)):
        ...
        if end_time > video_file_duration:
            ...
            if detect_segmets(segment,model,interval,person_threshold,time_threshold):
                segment_audio = segment.audio

                segment_path = os.path.join(output_path,f"{name_start}_high_quanlity_clip_{count_list[0]}.mp4")
                segment_audio_path = os.path.join(output_path, f"{name_start}_high_quanlity_clip_{count_list[0]}.m4a")
                count_list[0]+=1
...
def detect_segmets(segment,model,interval=2,person_threshold=0.3,time_threshold=0.8):


    calculate_frame_count=0 # 参与计算的帧数
    correct_frame_count=0 # 符合条件的帧数

    fps = segment.fps  # 获取帧率
    frame_count = int(segment.duration * fps)  # 计算总的帧数
    ...
    for frame_index in range(frame_count):
        if frame_index % interval ==0:
            ...
            frame=segment.get_frame(frame_index/fps)
            frame_bgr=cv2.cvtColor(frame,cv2.COLOR_RGB2BGR)

            # 使用模型进行检测
            results = model(frame_bgr)  # yolov5要的是bgr格式的图像

            # results中包含了检测结果的边界框信息，置信度分数，对象的类别标签信息
            persons = [x for x in results.xyxy[0].cpu().numpy() if int(x[5] == 0)]  # 类别 0 是person

            if (len(persons)==1): # 只有检测到一个人的时候才进行计算有效帧
                # 计算每帧中的人物面积
...
def parse_args():
    ...
    parse.add_argument("--in_dirs",type=str,help="输入视频文件夹路径",default="/data/shipu/video_src_h264_1/full_video")
    parse.add_argument("--out_dirs",type=str,help="输出视频文件夹路径",default="/data/shipu/video_src_h264_1/part_video")
    parse.add_argument("--interval",type=int,help="检测帧的间隔",default=2)
    parse.add_argument("--person_threshold",type=float,help="人物面积占视频帧的比例",default=0.25)
    parse.add_argument("--time_threshold",type=float,help="满足条件的视频帧所占的时长比例",default=0.9)
    parse.add_argument("--thread_num",type=int,help="线程数",default=4)
...
def processing(in_dirs,out_dirs,start,end,device):
    ...
    for in_dirs_subfolder in in_dirs_subfolders:
        ...
        # 这里我们修改一下逻辑，对无专场视频进行切割并进行检测，简单来讲，就是将无转场视频切分为5秒一段，如果该五秒钟的视频满足条件，就将其保存segements文件夹下
        no_transition_video_list = os.listdir(no_transition_video_path)

        good_video_clip_count=[0]
        ...
        for no_transition_video in no_transition_video_list:
            ...
            # (video_path,model,output_path,segment_duration=5.0,interval=2,person_threshold=0.3,time_threshold=0.8)
            split_video_and_detect_person(need_check_video_path,model,output_path=segments_path,segment_duration=5.0,interval=args.interval,person_threshold=args.person_threshold, time_threshold=args.time_threshold,count_list=good_video_clip_count)
            if good_video_clip_count[0]==100:
                break



        # 删除no_transition_video_path的所有文件
