import os
import sys
__dir__ = os.path.dirname(os.path.abspath(__file__)) # 获取当前文件所在的文件下
sys.path.append(__dir__)  # 将文件夹地址加入sys.path
sys.path.insert(0, os.path.abspath(os.path.join(__dir__, "..")))  #os.path.abspath(os.path.join(__dir__, "..")) 是当前文件夹的上一级文件夹，在本项目中也就是根目录了
...
from re_motion_module import get_motion_module
...
class PoseGuider(nn.Module):
    ...
    def forward(
            self, face,keypoint,hand_mesh,hdkp,
            temb=None, encoder_hidden_states=None, video_length=-1
    ):
        cond = torch.cat((face,keypoint,hand_mesh,hdkp), dim=1)
...
def get_poseguider(image_finetune=False,num_conds=4,cfg_path="/home/shipu/mycode/redance/config.yaml"):
    """
    第一阶段需要image_finetune,
    """
    cfg=OmegaConf.load(cfg_path)
    pose_guider_kwargs_dict=OmegaConf.to_container(cfg.pose_guider_kwargs.args)
    if  not image_finetune:
        unet_additional_kwargs_dict = OmegaConf.to_container(cfg.unet_additional_kwargs)
    ...
    poseguider=PoseGuider(image_finetune=image_finetune,num_conds=num_conds,**pose_guider_kwargs_dict)
    return poseguider
...
if __name__=="__main__":
    ...
    # pose_guider_kwargs_dict["motion_module_kwargs"] = unet_additional_kwargs_dict["motion_module_kwargs"]
    #
    # poseguider=PoseGuider(image_finetune=False,num_conds=3,**pose_guider_kwargs_dict).to("cuda")

    # 第二阶段image_finetune=False
    pose_guider=get_poseguider(image_finetune=False,num_conds=4).to("cuda")
    # print(pose_guider)
    ...
    face=torch.randn(size=[4,3,4,512,512]).to("cuda")
    body=torch.randn(size=[4,3,4,512,512]).to("cuda")
    hand=torch.randn(size=[4,3,4,512,512]).to("cuda")
    hdkp=torch.randn(size=[4,3,4,512,512]).to("cuda")

    frame=face.shape[2]
    face=rearrange(face,"b c f h w -> (b f) c h w")
    body=rearrange(body,"b c f h w -> (b f) c h w")
    hand=rearrange(hand,"b c f h w -> (b f) c h w")
    hdkp=rearrange(hdkp,"b c f h w -> (b f) c h w")

    result=pose_guider(face,body,hand,hdkp)

    result=[rearrange(pe,"(b f) c h w -> b c f h w",f=frame) for pe in result]
    for i in result:
        print(i.shape)
