...
def main(cfg):
    ...
    # 初始化reference_unet
    reference_unet = UNet2DConditionModel.from_pretrained(cfg.base_model_path,subfolder="unet").to(device="cuda")
    # 初始化denoising_unet
    denoising_unet=UNet3DConditionModel.from_pretrained_2d(cfg.base_model_path,"",subfolder="unet",
                                                           unet_additional_kwargs={"use_motion_module": False,
                                                                                   "unet_use_temporal_attention": False,},).to(device="cuda")
    # 图像编码器
    ...
    face_pose_guider=poseguider().to(device="cuda")
    body_pose_guider=poseguider().to(device="cuda")
    hand_pose_guider=poseguider().to(device="cuda")
    ...
    for epoch in range (first_epoch,num_train_epochs):
        ...
        for step,batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                pixel_values=batch["pixel_values"].to(weight_dtype) # 这是原始视频帧
...
# accelerate launch --config_file /home/shipu/mycode/one_gpu_default_config.yaml train_1_6.py
