...
from AniPortrait.src.utils.util import seed_everything,import_filename
...
from dataset_related.dataset_write_7_2 import MyDataset,collate_fn# 这里别忘记改了
...
class Net(nn.Module):
    ...
    def forward(self,
        ...
        pose_fea=self.pose_guider(pose_mesh_tensor,pose_keypoint_tensor,pose_hand_tensor,pose_hdkp_tensor,encoder_hidden_states=clip_image_embeds,temb=time_steps,video_length=frame)
...
def main(cfg):
    ...
    sample_dir = os.path.join(save_dir, 'samples')  # 其实这个文件夹可有可无

    if accelerator.is_main_process and not os.path.exists(sample_dir):
        os.makedirs(sample_dir)

    inference_config_path = "/home/shipu/mycode/AniPortrait/configs/inference/inference_v2.yaml"
    ...
    # 图像编码器
    image_enc = CLIPVisionModelWithProjection.from_pretrained(cfg.image_encoder_path).to(dtype=weight_dtype,
    ...
    denoising_unet = UNet3DConditionModel.from_pretrained_2d(cfg.base_model_path, cfg.mm_path, subfolder="unet",unet_additional_kwargs=OmegaConf.to_container(infer_config.unet_additional_kwargs), ).to(device=accelerator.device)

    # 用realisedance的poseguider
    pose_guider = get_poseguider(image_finetune=False,num_conds=4).to(device=accelerator.device)  # 第一阶段image_finetune为true
    ...
    for name,module in pose_guider.named_modules():
        if "motion_module" in name:
            for params in module.parameters():
                params.requires_grad=True
    ...
    for epoch in range (first_epoch,num_train_epochs):
        ...
        for step,batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                #                 pose_image_mesh,
                #                 pose_image_keypoint,
                pose_img_mesh=batch["pixel_values_mesh"]  # [b f c h w]
                ...
                pose_image_hands=pose_image_hands.transpose(1,2) # 新增的手部mesh

                pose_image_hdkp=batch["pixel_values_hdkp"]
                pose_image_hdkp=pose_image_hdkp.transpose(1,2)
        ...
        if accelerator.is_main_process:
            ...
            save_checkpoint(model=unwrap_net.denoising_unet, save_dir=save_dir, prefix="denoising_unet",
                            ckpt_num=global_step, total_limit=3)
            save_checkpoint(model=unwrap_net.pose_guider, save_dir=save_dir, prefix="pose_guider",
                            ckpt_num=global_step, total_limit=3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="/home/shipu/mycode/train_config/train_2.yaml")
...
# CUDA_VISIBLE_DEVICES=5,6,7 accelerate launch --config_file /home/shipu/mycode/default_config.yaml train_2_17.py
# CUDA_VISIBLE_DEVICES=6 accelerate launch --config_file /home/shipu/mycode/one_gpu_default_config.yaml train_2_17.py
