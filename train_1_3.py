...
from dataset_related.dataset_write_3 import MyDataset,collate_fn # 这里别忘记改了
"""
train_dataloader在不同的训练阶段是有不同表现形式的，在第一阶段没有引入motion module的时候我们采用的其实是图片-图片的训练方式
"""

"""
在这里，我们dataset_write_3，我们新增加了手部关键点的控制
在新的数据集写法中，我们新增了手部的关键点，同样的，我们首先尝试的还是将其放入到pose_guider中，得到特征和mesh以及kp特征相加求和
"""
...
class Net(nn.Module):
    ...
    def forward(self,
        ...
        hand_fea=self.pose_guider(pose_hand_tensor,ref_hand_tensor)  # 这里是新增的手部关键点
    ...
    for epoch in range (first_epoch,num_train_epochs):
        ...
        for step,batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                pose_image_hand=batch["pixel_values_hand"]
                pose_image_hand=pose_image_hand.unsqueeze(2)  # 新增的手部关键点 pose_hand

                # 这是不需要升维的
                ref_img_mesh=batch["pixel_values_ref_mesh"]
                ref_img_keypoint=batch["pixel_values_ref_keypoint"]
                ref_image_hand=batch["pixel_values_ref_hand"]  # ref_hand
                ...
                model_pred=net(noisy_latents,timesteps,ref_img_latents,image_prompt_embeds,ref_img_mesh,ref_img_keypoint,ref_image_hand,pose_img_mesh,pose_img_keypoint,pose_image_hand)# 这里先欠着，后来检查的时候这里应该是少了一个uncond的参数
...
# accelerate launch --config_file /remote-home/yfsong/shipu/mycode/one_gpu_default_config.yaml train_1_3.py
