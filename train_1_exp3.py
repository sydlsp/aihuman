...
from redance.poseguider_2cond import get_poseguider
...
class Net(nn.Module):
    ...
    def forward(self,
        ...
        pose_fea=self.pose_guider(pose_mesh_tensor,pose_keypoint_tensor)
...
def main(cfg):
    ...
    pose_guider=get_poseguider(image_finetune=True,num_conds=2).to(device=accelerator.device)  # 第一阶段image_finetune为true
    ...
    for epoch in range (first_epoch,num_train_epochs):
        ...
        for step,batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                # pose_image_hdkp=batch["pixel_values_hdkp"]  # [bs,c,h,w]
                # pose_image_hdkp=pose_image_hdkp.unsqueeze(2)  # 手部完整mesh

                # 这是不需要升维的
                ref_img_mesh=batch["pixel_values_ref_mesh"]
                ref_img_keypoint=batch["pixel_values_ref_keypoint"]
                ref_image_hdkp=batch["pixel_values_ref_hdkp"]  # ref_hand
                ...
                model_pred=net(noisy_latents,timesteps,ref_img_latents,image_prompt_embeds,pose_img_mesh,pose_img_keypoint)# 这里先欠着，后来检查的时候这里应该是少了一个uncond的参数
...
# CUDA_VISIBLE_DEVICES=6,7 accelerate launch --config_file /home/shipu/mycode/default_config.yaml train_1_exp3.py
# CUDA_VISIBLE_DEVICES=6 accelerate launch --config_file  /home/shipu/mycode/one_gpu_default_config.yaml train_1_13.py

# ssh -i /Users/shipu/Documents/C/rsa/A6000 -p 26580 shipu@10.176.53.120
