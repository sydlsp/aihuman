...
class PoseGuider(ModelMixin):
    def __init__(self, noise_latent_channels=320, use_ca=True):
        ...
        self.conv_layers_1 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(in_channels=noise_latent_channels, out_channels=noise_latent_channels, kernel_size=3, stride=2,
                      padding=1),
            nn.BatchNorm2d(noise_latent_channels),
            nn.ReLU(),
        )
        ...
        self.conv_layers_2 = nn.Sequential(
            ...
            nn.BatchNorm2d(noise_latent_channels),
            nn.ReLU(),
            nn.Conv2d(in_channels=noise_latent_channels, out_channels=noise_latent_channels * 2, kernel_size=3,
                      stride=2, padding=1),
        ...
        self.conv_layers_3 = nn.Sequential(
            nn.Conv2d(in_channels=noise_latent_channels * 2, out_channels=noise_latent_channels * 2, kernel_size=3,
                      padding=1),
            nn.BatchNorm2d(noise_latent_channels * 2),
            nn.ReLU(),
            nn.Conv2d(in_channels=noise_latent_channels * 2, out_channels=noise_latent_channels * 4, kernel_size=3,
                      stride=2, padding=1),
        ...
        self.conv_layers_4 = nn.Sequential(
            nn.Conv2d(in_channels=noise_latent_channels * 4, out_channels=noise_latent_channels * 4, kernel_size=3,
                      padding=1),
            nn.BatchNorm2d(noise_latent_channels * 4),
            nn.ReLU(),
        )
    ...
    def forward(self, x, ref_x):
        ...
        if self.use_ca:
            ...
            x = 0.2*self.cross_attn1(x, ref_x)+0.8*x
        fea.append(rearrange(x, "(b f) c h w -> b c f h w", b=b))

        x = self.conv_layers_2(x)
        if self.use_ca:
            ref_x = self.conv_layers_2(ref_x)
            x = 0.2*self.cross_attn2(x, ref_x)+0.8*x
        fea.append(rearrange(x, "(b f) c h w -> b c f h w", b=b))

        x = self.conv_layers_3(x)
        if self.use_ca:
            ref_x = self.conv_layers_3(ref_x)
            x = 0.2*self.cross_attn3(x, ref_x)+0.8*x
        fea.append(rearrange(x, "(b f) c h w -> b c f h w", b=b))

        x = self.conv_layers_4(x)
        if self.use_ca:
            ref_x = self.conv_layers_4(ref_x)
            x = 0.2*self.cross_attn4(x, ref_x)+0.8*x
        fea.append(rearrange(x, "(b f) c h w -> b c f h w", b=b))

        return fea

    @classmethod
    def from_pretrained(cls, pretrained_model_path, Hack_PoseGuider=None):
