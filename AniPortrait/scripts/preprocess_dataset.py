...
# 从视频中提取关键点并且编写训练用的json文件
def main():
    args = parse_args()

    # exist_ok=True 意味着当文件夹已经存在的时候不会抛出异常，而是直接返回
    os.makedirs(args.output_dir, exist_ok=True)
    # os.scandir是一个迭代器，遍历args.input_dir目录下所有文件和子文件夹
    # 如果是文件夹，就把文件夹的路径加入到folders列表中
    folders = [f.path for f in os.scandir(args.input_dir) if f.is_dir()]
    folders.sort()

    # 创建一个人脸关键点提取器LMKExtractor()类的实例
    ...
    for folder in folders:
        ...
        for img_file in sorted(glob.glob(os.path.join(folder, "*.png"))):
            ...
            # 对图片提取关键点信息
            # result中包括了关键点坐标[lmks]、3D关键点坐标[lmks3d]、变换矩阵[trans_mat]、面部混合形状得分[bs]
