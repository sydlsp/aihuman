...
from AniPortrait.src.models.pose_guider_1 import PoseGuider as poseguider  # poseguider很重要别忘了改了
...
"""
在这个版本中，我们增加了poseguider的输出中x的比重，减少了ref_x的比重，希望模型更多的关注到x原本的信息
"""
"""
使用卷积网络对三个特征进行融合，在这里我们加上残差连接，强调手部条件信息
在这一版中我们进一步增加hand_mesh在融合过程中的比重，希望模型能狗更加关注到手部信息
"""
class FusionNet(nn.Module):
    ...
    def forward(self,mesh_list,body_list,hand_list):
        a=0.3*self.layer_1(torch.cat([mesh_list[0],body_list[0],hand_list[0]],dim=1))+0.7*hand_list[0]
        b=0.3*self.layer_2(torch.cat([mesh_list[1],body_list[1],hand_list[1]],dim=1))+0.7*hand_list[1]
        c=0.3*self.layer_3(torch.cat([mesh_list[2],body_list[2],hand_list[2]],dim=1))+0.7*hand_list[2]
        d=0.3*self.layer_4(torch.cat([mesh_list[3],body_list[3],hand_list[3]],dim=1))+0.7*hand_list[3]
        e=0.3*self.layer_5(torch.cat([mesh_list[4],body_list[4],hand_list[4]],dim=1))+0.7*hand_list[4]
        return [a,b,c,d,e]
...
# CUDA_VISIBLE_DEVICES=5 accelerate launch --config_file  /home/shipu/mycode/one_gpu_default_config.yaml train_1_11.py
