...
class WeightFusionNet(nn.Module):
    ...
    def forward(self, mesh_fea, keypoint_fea, hand_fea):
        normalized_weights = torch.softmax(self.weights, dim=0)  # 归一化权重
        fused_fea = normalized_weights[0] * mesh_fea + normalized_weights[1] * keypoint_fea + normalized_weights[2] * hand_fea
        return fused_fea
...
class Net(nn.Module):
    ...
    def forward(self,
        ...
        pose_fea=[self.fusion_net(x,y,z) for x,y,z in zip(mesh_fea,keypoint_fea,hand_fea)]
        #
        # for i in pose_fea:
        #     print(i.shape)
...
def main(cfg):
    ...
    # 初始化fusion_net
    fusion_net=WeightFusionNet().to(device=accelerator.device)
    ...
    for epoch in range (first_epoch,num_train_epochs):
        ...
        if (epoch+1)% cfg.save_model_epoch_interval==0 and accelerator.is_main_process:
            ...
            save_checkpoint(model=unwrap_net.fusion_net, save_dir=save_dir, prefix="fusion_net",ckpt_num=global_step, total_limit=3)
...
# CUDA_VISIBLE_DEVICES=3 accelerate launch --config_file  /remote-home/yfsong/shipu/mycode/one_gpu_default_config_deepspeed.yaml train_1.py
