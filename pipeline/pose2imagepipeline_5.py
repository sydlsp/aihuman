...
这个版本是mesh_fea+kp_fea+hand_fea=pose_fea的版本，同时，我们加上了文本引导
...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def __call__(self,
        ...
        # 在这里我们加上text_embeds的结果
        clip_image_embeds=clip_image_embeds+text_embedding.to(device)
...
# 文本编码器和分词器
text_enc = CLIPTextModel.from_pretrained("/remote-home/share/yfsong/shipu/pretrained_model/text_encoder").to(device=device)
...
text_input = tokenizer(prompt_text, return_tensors="pt").to(device=device)
...
ckpt_path="/remote-home/share/yfsong/shipu/test_output/stage1_6/checkpoint-100000/pytorch_model.bin"
...
for i in range(0,min(10,len(pose_mesh_list))):
    ...
    image=pipeline(ref_image=ref_frame_pil,pose_mesh=pose_mesh_pil,pose_keypoint=pose_keypoint_pil,pose_hand=pose_hand_pil,ref_mesh=ref_mesh_pil,ref_keypoint=ref_keypoint_pil,ref_hand=ref_hand_pil,width=cfg.data.sample_size[0],height=cfg.data.sample_size[1],num_inference_steps=250,guidance_scale=3.5,generator=generator,text_embedding=text_embedding).images
