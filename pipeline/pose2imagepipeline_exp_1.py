...
from redance.poseguider import get_poseguider
from train_1_exp1 import Net   # 修改一下Net的来源
...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def __call__(self,
        ...
        mesh_cond_tensor=mesh_cond_tensor.to(device=device)


        keypoint_cond_tensor=keypoint_cond_tensor.unsqueeze(2)
        keypoint_cond_tensor=keypoint_cond_tensor.to(device=device)

        hand_cond_tensor=hand_cond_tensor.unsqueeze(2)
        hand_cond_tensor=hand_cond_tensor.to(device=device)
        ...
        frame=mesh_cond_tensor.shape[2]
        mesh_cond_tensor=rearrange(mesh_cond_tensor,"b c f h w -> (b f) c h w")
        keypoint_cond_tensor=rearrange(keypoint_cond_tensor,"b c f h w -> (b f) c h w")
        hand_cond_tensor=rearrange(hand_cond_tensor,"b c f h w -> (b f) c h w")
        pose_fea=self.pose_guider(mesh_cond_tensor,keypoint_cond_tensor,hand_cond_tensor)
...
pose_guider=get_poseguider(image_finetune=True,num_conds=3).to(device)
...
ckpt_path="/data/shipu/ani_train_ckpt/exp_1_stage1/checkpoint-200000/pytorch_model.bin"
...
#/data/shipu/train_data/data/shipu/dataset_true/videos/video_198_high_quanlity_clip_95
pose_dir="/data/shipu/train_data_full/train_data_hdkp/videos/video_3_high_quanlity_clip_84"
...
pose_hand_list=[i for i in os.listdir(pose_dir) if "_full_" in i]
...
# 准备好ref_img的相关图片
ref_frame_path="/data/shipu/train_data_full/train_data_hdkp/videos/video_116_high_quanlity_clip_15/video_116_high_quanlity_clip_15_frame_0000.jpg"
# /data/shipu/train_data_hdkp/videos/video_106_high_quanlity_clip_74/video_106_high_quanlity_clip_74_frame_0000.jpg
#/data/shipu/train_data/data/shipu/dataset_true/videos/video_3_high_quanlity_clip_84/video_3_high_quanlity_clip_84_frame_0000.jpg
# /data/shipu/train_data/data/shipu/dataset_true/videos/video_169_high_quanlity_clip_22/video_169_high_quanlity_clip_22_frame_0039.jpg
# /data/shipu/train_data/data/shipu/dataset_true/videos/video_198_high_quanlity_clip_49/video_198_high_quanlity_clip_49_frame_0000.jpg
...
for i in range(0,min(30,len(pose_mesh_list))):
# for i in range(6,15):
    # pose
    ...
    image=pipeline(ref_image=ref_frame_pil,pose_mesh=pose_mesh_pil,pose_keypoint=pose_keypoint_pil,pose_hand=pose_hand_pil,width=cfg.data.sample_size[0],height=cfg.data.sample_size[1],num_inference_steps=50,guidance_scale=2,generator=generator).images
    image = image[0, :, 0].permute(1, 2, 0).cpu().numpy()  # (3, h, w)
    res_image_pil = Image.fromarray((image * 255).astype(np.uint8))
    save_dir="/home/shipu/mycode/output/save_125"
    os.makedirs(save_dir,exist_ok=True)
    res_image_pil.save(os.path.join(save_dir,f"res_image_{i}.jpg"))
