...
from my_module.new_poseguider import new_poseguider_2 as new_poseguider
...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def __call__(self,
        ...
        # 将上面准备好的东西送到pose_guider中,pose_guider先ref后cond，先mesh后keypoint
        pose_fea=self.pose_guider(ref_mesh_tensor,ref_keypoint_tensor,mesh_cond_tensor,keypoint_cond_tensor)
...
# 换poseguider的时候不要忘了在最上面把poseguider给换了
...
ckpt_path="/remote-home/share/yfsong/shipu/test_output/stage1_1/checkpoint-300000/pytorch_model.bin"
...
# reference_unet_ckpt_path="/remote-home/share/yfsong/shipu/test_output/stage1_2/reference_unet-99940.pth"
# denoising_unet_ckpt_path="/remote-home/share/yfsong/shipu/test_output/stage1_2/denoising_unet-99940.pth"
# pose_guider_ckpt_path="/remote-home/share/yfsong/shipu/test_output/stage1_2/pose_guider-99940.pth"
#
...
# 以下注释的部分是对单张进行推理
# ref_img_path="/remote-home/share/yfsong/shipu/dataset_video_true/shengyu/video_2_high_quanlity_clip_90/video_2_high_quanlity_clip_90_frame_0000.jpg"
# ref_mesh_path="/remote-home/share/yfsong/shipu/dataset_video_true/shengyu/video_2_high_quanlity_clip_90/video_2_high_quanlity_clip_90_mesh_0000.jpg"
# ref_keypoint_path="/remote-home/share/yfsong/shipu/dataset_video_true/shengyu/video_2_high_quanlity_clip_90/video_2_high_quanlity_clip_90_keypoint_0000.jpg"
#
# ref_img=cv2.imread(ref_img_path)
# ref_img=cv2.cvtColor(ref_img,cv2.COLOR_BGR2RGB)
# ref_img_pil=Image.fromarray(ref_img).convert('RGB')
#
...
ref_dir="/remote-home/share/yfsong/shipu/dataset_video_true/shengyu/video_2_high_quanlity_clip_90"

frame_list=[i for i in os.listdir(ref_dir) if "frame" in i]
keypoint_list=[i for i in os.listdir(ref_dir) if "keypoint" in i]
mesh_list=[i for i in os.listdir(ref_dir) if "mesh" in i]

def extract_number(filename):
    return int(filename.split(".")[0].split("_")[-1])

frame_list=sorted(frame_list,key=extract_number)
keypoint_list=sorted(keypoint_list,key=extract_number)
mesh_list=sorted(mesh_list,key=extract_number)

# 准备好pose_img的相关图片
pose_mesh_path="/remote-home/share/yfsong/shipu/dataset_video_true/shengyu/video_8_high_quanlity_clip_30/video_8_high_quanlity_clip_30_mesh_0000.jpg"
pose_keypoint_path="/remote-home/share/yfsong/shipu/dataset_video_true/shengyu/video_8_high_quanlity_clip_30/video_8_high_quanlity_clip_30_keypoint_0000.jpg"
...
# 单张推理
# image=pipeline(ref_image=ref_img_pil,pose_mesh=pose_mesh_pil,pose_keypoint=pose_keypoint_pil,ref_mesh=ref_mesh_pil,ref_keypoint=ref_keypoint_pil,width=cfg.data.sample_size[0],height=cfg.data.sample_size[1],num_inference_steps=25,guidance_scale=3.5,generator=generator).images
# image = image[0, :, 0].permute(1, 2, 0).cpu().numpy()  # (3, h, w)
# res_image_pil = Image.fromarray((image * 255).astype(np.uint8))
# res_image_pil.save("/remote-home/yfsong/shipu/mycode/save/res_image_111222333444.jpg")
# print(res_image_pil)

for i in range(len(frame_list)):
    ref_img_path=os.path.join(ref_dir,frame_list[i])
    ref_mesh_path=os.path.join(ref_dir,mesh_list[i])
    ref_keypoint_path=os.path.join(ref_dir,keypoint_list[i])
    ...
    image=pipeline(ref_image=ref_img_pil,pose_mesh=pose_mesh_pil,pose_keypoint=pose_keypoint_pil,ref_mesh=ref_mesh_pil,ref_keypoint=ref_keypoint_pil,width=cfg.data.sample_size[0],height=cfg.data.sample_size[1],num_inference_steps=250,guidance_scale=3.5,generator=generator).images
    image = image[0, :, 0].permute(1, 2, 0).cpu().numpy()  # (3, h, w)
    res_image_pil = Image.fromarray((image * 255).astype(np.uint8))
    res_image_pil.save(f"/remote-home/yfsong/shipu/mycode/output/save_1/res_image_{i}.jpg")
