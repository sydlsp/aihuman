...
配合dataset_4 train1_6使用
我们将poseguider拆开分成了三个不同的poseguider
...
from AniPortrait.src.models.pose_guider_1 import PoseGuider as new_poseguider
from train_1_11 import Net,FusionNet   # 修改一下Net的来源
...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def __init__(self,vae,image_encoder,reference_unet,denoising_unet,face_pose_guider,body_pose_guider,hand_pose_guider,fusion_net,
                 scheduler:Union[
                        DDIMScheduler,
                        DPMSolverMultistepScheduler,
                        EulerAncestralDiscreteScheduler,
        ...
        self.register_modules(
            ...
            face_pose_guider=face_pose_guider,
            body_pose_guider=body_pose_guider,
            hand_pose_guider=hand_pose_guider,
    ...
    def __call__(self,
        ...
        mesh_cond_tensor=mesh_cond_tensor.to(device=device,dtype=self.face_pose_guider.dtype)


        keypoint_cond_tensor=keypoint_cond_tensor.unsqueeze(2)
        keypoint_cond_tensor=keypoint_cond_tensor.to(device=device,dtype=self.body_pose_guider.dtype)

        hand_cond_tensor=hand_cond_tensor.unsqueeze(2)
        hand_cond_tensor=hand_cond_tensor.to(device=device,dtype=self.hand_pose_guider.dtype)
        ...
        ref_mesh_tensor=ref_mesh_tensor.to(device=device,dtype=self.face_pose_guider.dtype)
        ref_keypoint_tensor=ref_keypoint_tensor.to(device=device,dtype=self.body_pose_guider.dtype)

        """新加的手部关键点"""
        ref_hand_tensor=self.cond_image_processor.preprocess(ref_hand,height=height,width=width)
        ref_hand_tensor=ref_hand_tensor.to(device=device,dtype=self.hand_pose_guider.dtype)

        # 在这里得到三个fearure
        mesh_fea=self.face_pose_guider(mesh_cond_tensor,ref_mesh_tensor)
        keypoint_fea=self.body_pose_guider(keypoint_cond_tensor,ref_keypoint_tensor)
        hand_fea=self.hand_pose_guider(hand_cond_tensor,ref_hand_tensor)
...
device="cuda"
# 先把要用的基础模型加进来
cfg=OmegaConf.load("/home/shipu/mycode/train_config/train_1_para.yaml")
# 初始化VAE
...
face_pose_guider=new_poseguider().to(device=device)
body_pose_guider=new_poseguider().to(device=device)
hand_pose_guider=new_poseguider().to(device=device)
fusion_net=FusionNet().to(device=device)


model=Net(reference_unet,denoising_unet,face_pose_guider,body_pose_guider,hand_pose_guider,fusion_net,reference_control_writer,reference_control_reader)


# 这是从bin文件中直接把所有权重包装好的直接拿
ckpt_path="/data/shipu/ani_train_ckpt/stage1_9/checkpoint-68000/pytorch_model.bin"
...
face_pose_guider = model.face_pose_guider
body_pose_guider=model.body_pose_guider
hand_pose_guider=model.hand_pose_guider
fusion_net=model.fusion_net
#
...
pipeline=Pose2ImagePipeline(vae=vae,image_encoder=image_enc,reference_unet=reference_unet,denoising_unet=denoising_unet,face_pose_guider=face_pose_guider,body_pose_guider=body_pose_guider,hand_pose_guider=hand_pose_guider,fusion_net=fusion_net,scheduler=scheduler)


pose_dir="/data/shipu/data/shipu/dataset_true/videos/video_3_high_quanlity_clip_84"
...
# 准备好ref_img的相关图片
ref_frame_path="/data/shipu/data/shipu/dataset_true/videos/video_3_high_quanlity_clip_84/video_3_high_quanlity_clip_84_frame_0000.jpg"
ref_mesh_path="/data/shipu/data/shipu/dataset_true/videos/video_3_high_quanlity_clip_84/video_3_high_quanlity_clip_84_mesh_0000.jpg"
ref_keypoint_path="/data/shipu/data/shipu/dataset_true/videos/video_3_high_quanlity_clip_84/video_3_high_quanlity_clip_84_keypoint_0000.jpg"
ref_hand_path="/data/shipu/data/shipu/dataset_true/videos/video_3_high_quanlity_clip_84/video_3_high_quanlity_clip_84_hands_0000.jpg"  # 这里补上hands的路径
...
# for i in range(0,min(50,len(pose_mesh_list))):
for i in range (92,102):
    ...
    image=pipeline(ref_image=ref_frame_pil,pose_mesh=pose_mesh_pil,pose_keypoint=pose_keypoint_pil,pose_hand=pose_hand_pil,ref_mesh=ref_mesh_pil,ref_keypoint=ref_keypoint_pil,ref_hand=ref_hand_pil,width=cfg.data.sample_size[0],height=cfg.data.sample_size[1],num_inference_steps=50,guidance_scale=1.5,generator=generator).images
    image = image[0, :, 0].permute(1, 2, 0).cpu().numpy()  # (3, h, w)
    res_image_pil = Image.fromarray((image * 255).astype(np.uint8))
    res_image_pil.save(f"/home/shipu/mycode/output/save_33/res_image_{i}.jpg")
