...
from train_1_7 import Net   # 修改一下Net的来源
...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def __init__(self,vae,image_encoder,reference_unet,denoising_unet,face_pose_guider,body_pose_guider,hand_pose_guider,
                 scheduler:Union[
                        DDIMScheduler,
                        DPMSolverMultistepScheduler,
                        EulerAncestralDiscreteScheduler,
    ...
    def __call__(self,
        ...
        pose_fea=[x+y+z for x,y,z in zip(mesh_fea,keypoint_fea,hand_fea)]
...
device="cuda"
# 先把要用的基础模型加进来
cfg=OmegaConf.load("/home/shipu/mycode/train_config/train_1.yaml")
# 初始化VAE
...
model=Net(reference_unet,denoising_unet,face_pose_guider,body_pose_guider,hand_pose_guider,reference_control_writer,reference_control_reader)


# 这是从bin文件中直接把所有权重包装好的直接拿
ckpt_path="/data/shipu/ani_train_ckpt/stage1_5/checkpoint-50000/pytorch_model.bin"
...
pipeline=Pose2ImagePipeline(vae=vae,image_encoder=image_enc,reference_unet=reference_unet,denoising_unet=denoising_unet,face_pose_guider=face_pose_guider,body_pose_guider=body_pose_guider,hand_pose_guider=hand_pose_guider,scheduler=scheduler)
...
for i in range(0,min(50,len(pose_mesh_list))):
    ...
    res_image_pil.save(f"/home/shipu/mycode/output/save_20/res_image_{i}.jpg")
