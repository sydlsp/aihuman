...
from train_1_9 import Net,FusionNet   # 修改一下Net的来源
...
ckpt_path="/data/shipu/ani_train_ckpt/stage1_6/checkpoint-100000/pytorch_model.bin"
...
for i in range(0,min(50,len(pose_mesh_list))):
    ...
    image=pipeline(ref_image=ref_frame_pil,pose_mesh=pose_mesh_pil,pose_keypoint=pose_keypoint_pil,pose_hand=pose_hand_pil,ref_mesh=ref_mesh_pil,ref_keypoint=ref_keypoint_pil,ref_hand=ref_hand_pil,width=cfg.data.sample_size[0],height=cfg.data.sample_size[1],num_inference_steps=150,guidance_scale=2.5,generator=generator).images
    image = image[0, :, 0].permute(1, 2, 0).cpu().numpy()  # (3, h, w)
    res_image_pil = Image.fromarray((image * 255).astype(np.uint8))
    res_image_pil.save(f"/home/shipu/mycode/output/save_22/res_image_{i}.jpg")
