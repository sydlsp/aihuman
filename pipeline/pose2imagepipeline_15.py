...
from train_1_15 import Net   # 修改一下Net的来源
...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def __call__(self,
        ...
        hand_cond_tensor=self.cond_image_processor.preprocess(pose_hand,height=height,width=width)  # 新加的手的mesh
        hdkp_cond_tensor=self.cond_image_processor.preprocess(pose_hdkp,height=height,width=width)  # 新加的手的关键点
        ...
        hdkp_cond_tensor=hdkp_cond_tensor.unsqueeze(2)
        hdkp_cond_tensor=hdkp_cond_tensor.to(device=device)
        ...
        pose_fea=self.pose_guider(mesh_cond_tensor,keypoint_cond_tensor,hand_cond_tensor,hdkp_cond_tensor)
...
#初始化denoising_unet
denoising_unet=UNet3DConditionModel.from_pretrained_2d(cfg.base_model_path,"",subfolder="unet",unet_additional_kwargs={"use_motion_module": False,"unet_use_temporal_attention": False,},).to(device=device)
...
pose_guider=get_poseguider(image_finetune=True,num_conds=4).to(device)
...
ckpt_path="/data/shipu/ani_train_ckpt/stage1_4cond_high/checkpoint-200000/pytorch_model.bin"
...
pose_dir="/data/shipu/train_data_hdkp/videos/video_198_high_quanlity_clip_49"
# /data/shipu/train_data_hdkp/videos/video_3_high_quanlity_clip_84/video_3_high_quanlity_clip_84_frame_0000.jpg
...
pose_hdkp_list=[i for i in os.listdir(pose_dir) if "hdkp" in i]
...
# 准备好ref_img的相关图片
ref_frame_path="/data/shipu/train_data_hdkp/videos/video_198_high_quanlity_clip_49/video_198_high_quanlity_clip_49_frame_0000.jpg"
# /data/shipu/train_data_hdkp/videos/video_3_high_quanlity_clip_84/video_3_high_quanlity_clip_84_frame_0000.jpg
# /data/shipu/train_data_hdkp/videos/video_198_high_quanlity_clip_49/video_198_high_quanlity_clip_49_frame_0000.jpg
...
# for i in range(92,102):
    ...
    pose_hdkp_path=os.path.join(pose_dir,pose_hdkp_list[i])
    ...
    pose_hdkp=cv2.imread(pose_hdkp_path)
    pose_hdkp=cv2.cvtColor(pose_hdkp,cv2.COLOR_BGR2RGB)
    pose_hdkp_pil=Image.fromarray(pose_hdkp).convert('RGB')
    ...
    image=pipeline(ref_image=ref_frame_pil,pose_mesh=pose_mesh_pil,pose_keypoint=pose_keypoint_pil,pose_hand=pose_hand_pil,pose_hdkp=pose_hdkp_pil,width=cfg.data.sample_size[0],height=cfg.data.sample_size[1],num_inference_steps=50,guidance_scale=2,generator=generator).images
    image = image[0, :, 0].permute(1, 2, 0).cpu().numpy()  # (3, h, w)
    res_image_pil = Image.fromarray((image * 255).astype(np.uint8))
    save_dir = "/home/shipu/mycode/output/save_64"
    os.makedirs(save_dir, exist_ok=True)
    res_image_pil.save(os.path.join(save_dir, f"res_image_{i}.jpg"))
    print(f"save image {i}")


# CUDA_VISIBLE_DEVICES=4 python pose2imagepipeline_15.py
