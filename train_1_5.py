...
"""
train1_5 在train1_4的基础上引入clip的文本编码器来做一些引导
"""
...
class Net(nn.Module):
    def __init__(self,
        ...
        self.pose_guider=pose_guider  # poseguider暂时就用一个
...
def main(cfg):
    ...
    # 这里我们新增文本编码器
    text_enc=CLIPTextModel.from_pretrained("/remote-home/share/yfsong/shipu/pretrained_model/text_encoder").to(dtype=weight_dtype,device="cuda")
    tokenizer=CLIPTokenizer.from_pretrained("/remote-home/share/yfsong/shipu/pretrained_model/text_encoder")

    prompt_text="A hyper-realistic full-body portrait of a human, with both hands clearly visible, each hand having exactly five fingers, ultra high resolution, detailed skin texture, accurate proportions,  photorealistic, sharp focus, intricate details, 8K quality"
    text_input=tokenizer(prompt_text,return_tensors="pt").to("cuda")
    text_embedding=text_enc(**text_input).pooler_output

    # 初始化poseguider，这里我们沿用本来的pose_guider只是在前面加一个3d卷积来对形状
    ...
    pose_guider.requires_grad_(True)  # pose_guider的参数是需要训练的
        ...
        for step,batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                with torch.no_grad():
                    ...
                    # 这里是把文本编码器的输出加到clip_img_embeds上
                    clip_img_embeds=clip_img_embeds+text_embedding
...
# accelerate launch --config_file /remote-home/yfsong/shipu/mycode/one_gpu_default_config.yaml train_1_5.py
